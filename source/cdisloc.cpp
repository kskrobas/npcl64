...
//position d;
                ...
                for(auto &atom: grain->atoms){
                StVector atom2vec(atom.x,atom.y,atom.z);
                position   d{pointAxisDistance(mainAxis,atom2vec)};

                    if(rmin<d && d<rmax){
                    //StRotationMatrix rotMat(frpy(vec_b,atom2vec),sa,ca);
                    StRotationMatrix rotMat(mainAxis,sa,ca);

                            fRotRndShift(rotMat,atom2vec,rndShift);
